...
Route::prefix('v1')->group(function () {
    ...
    Route::get('storage/{path}', function (string $path) {
        ...
        if (\Illuminate\Support\Facades\Storage::disk('public')->exists($path)) {
            $mime = \Illuminate\Support\Facades\Storage::disk('public')->mimeType($path) ?: 'image/webp';
            return response(\Illuminate\Support\Facades\Storage::disk('public')->get($path), 200, array_merge($headers, ['Content-Type' => $mime]));
        }

        if (\Illuminate\Support\Facades\Storage::disk('public')->exists(basename($path))) {
            $mime = \Illuminate\Support\Facades\Storage::disk('public')->mimeType(basename($path)) ?: 'image/webp';
            return response(\Illuminate\Support\Facades\Storage::disk('public')->get(basename($path)), 200, array_merge($headers, ['Content-Type' => $mime]));
        }
