...
class LogSeeder extends Seeder
{
    public function run(): void
    {
        ...
        // 4. Notification Logs (100 records)
        $notificationLogs = [];
        ...
        for ($i = 1; $i <= 100; $i++) {
            $notificationLogs[] = [
                ...
                'body' => "Dear user, this is system transaction alert $i. Please check your activity details.",
                'status' => 'sent',
                'meta' => null,
                'sent_at' => now()->subDays(30 - ($i / 3.3)),
                'created_at' => now()->subDays(30 - ($i / 3.3)),
                'updated_at' => now()->subDays(30 - ($i / 3.3)),
            ];
        }
        DB::table('notification_logs')->insert($notificationLogs);
    }
}
