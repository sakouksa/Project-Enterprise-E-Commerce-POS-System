...
const ProductFormPage: React.FC = () => {
  ...
  const validate = () => {
    ...
    if (!form.name.trim()) {
      errors.name = t('products.errors.nameRequired', 'សូមបញ្ចូលឈ្មោះទំនិញ')
    }
    if (!form.sku.trim()) {
      errors.sku = t('products.errors.skuRequired', 'សូមបញ្ចូលលេខកូដ SKU')
    }
    if (!form.selling_price || isNaN(parseFloat(form.selling_price))) {
      errors.selling_price = t('products.errors.priceRequired', 'សូមបញ្ចូលតម្លៃលក់')
    }
    return errors
  }
