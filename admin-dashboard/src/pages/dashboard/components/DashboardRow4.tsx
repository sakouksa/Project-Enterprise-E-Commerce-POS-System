    ...
    <div className="grid grid-cols-1 md:grid-cols-3 gap-6">
      ...
      <div className="bg-card border border-border/60 rounded-2xl p-5 shadow-sm flex flex-col justify-between">
        <div>
          ...
          <div className="space-y-3">
            ...
            <div className="flex justify-between items-center text-xs py-1.5">
              <span className="text-muted-foreground font-semibold">{t('dashboard.today_stock_movement', "Today's Stock Movements")}</span>
        ...
        <button 
          ...
          className="mt-4 text-[11px] text-primary hover:underline font-bold text-left"
        >
          {t('dashboard.viewAll')} →
        </button>
      </div>
            ...
            {lowStockList?.slice(0, 3).map((item, idx) => (
              <div key={idx} className="flex justify-between items-center text-xs border-b border-border/20 pb-2 last:border-0 last:pb-0">
                ...
                <span className="px-2 py-0.5 bg-rose-500/10 text-rose-500 text-[10px] font-bold rounded-full">
                  {item.quantity} {t('common.left', 'left')}
                </span>
              </div>
            ))}
      ...
      <div className="bg-card border border-border/60 rounded-2xl p-5 shadow-sm flex flex-col justify-between">
        <div>
          ...
          <div className="mt-4">
            <span className="text-xs text-muted-foreground font-semibold">{t('inventory.total_stock_assets', 'Total Stock Valuation')}</span>
