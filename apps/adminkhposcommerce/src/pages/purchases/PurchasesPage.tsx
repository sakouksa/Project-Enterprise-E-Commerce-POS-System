...
const PurchasesPage: React.FC = () => {
  ...
  const handleFormSubmit = (e: React.FormEvent) => {
    ...
    if (!supplierId) {
      errors.supplierId = t('purchases.errors.supplierRequired', 'សូមជ្រើសរើសអ្នកផ្គត់ផ្គង់')
    }
    if (!warehouseId) {
      errors.warehouseId = t('purchases.errors.warehouseRequired', 'សូមជ្រើសរើសឃ្លាំងទទួល')
    }
    if (!branchId) {
      errors.branchId = t('purchases.errors.branchRequired', 'សូមជ្រើសរើសសាខា')
    }
    if (!poDate) {
      errors.poDate = t('purchases.errors.dateRequired', 'សូមជ្រើសរើសកាលបរិច្ឆេទបញ្ជាទិញ')
    }
    ...
    if (formItems.length === 0) {
      toast.error(t('purchases.addProductRequired', 'សូមបញ្ចូលទំនិញយ៉ាងហោចណាស់មួយមុខ'))
      return
    }
    ...
    <div className="space-y-6">
      {activeWorkspaceTab === 'list' ? (
        <>
          ...
          {/* List Page Header */}
          <div className="flex flex-col md:flex-row md:items-center justify-between gap-4 py-1 print:hidden">
