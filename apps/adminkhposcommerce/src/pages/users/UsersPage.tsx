...
const UsersPage: React.FC = () => {
  ...
  const validate = () => {
    ...
    if (!name.trim()) {
      errors.name = t('users.errors.nameRequired', 'សូមបញ្ចូលឈ្មោះអ្នកប្រើប្រាស់')
    }
    if (!email.trim()) {
      errors.email = t('users.errors.emailRequired', 'សូមបញ្ចូលអ៊ីមែល')
    }
    if (!editingUser && !password.trim()) {
      errors.password = t('users.errors.passwordRequired', 'សូមបញ្ចូលពាក្យសម្ងាត់')
    }
    return errors
  }
