    ...
    <div className="space-y-6 pb-10 w-full">
      ...
      <form onSubmit={handleSubmit} className="space-y-6 w-full">
        ...
        <div className="bg-card dark:bg-slate-900 border border-border/80 dark:border-slate-800 rounded-2xl p-6 shadow-2xs space-y-5">
          ...
          <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-3 gap-4">
            ...
            <div>
              ...
              >
                <option value="distributor" className="dark:bg-slate-900">{t('suppliers.distributor', 'អ្នកចែកចាយ')}</option>
                <option value="manufacturer" className="dark:bg-slate-900">{t('suppliers.manufacturer', 'រោងចក្រផលិតផ្ទាល់')}</option>
                <option value="importer" className="dark:bg-slate-900">{t('suppliers.importer', 'តំណាងនាំចូលផ្លូវការ')}</option>
                <option value="service" className="dark:bg-slate-900">{t('suppliers.serviceProvider', 'សេវាកម្មដឹកជញ្ជូន')}</option>
                <option value="other" className="dark:bg-slate-900">{t('suppliers.other', 'ដៃគូពាណិជ្ជកម្មផ្សេងទៀត')}</option>
              </select>
            </div>
            ...
            <div>
              ...
              <select
                ...
                className="w-full h-10 min-h-[40px] px-3.5 py-2 text-xs sm:text-[13px] rounded-lg border border-border/80 dark:border-slate-700/80 bg-background dark:bg-slate-900/90 text-foreground dark:text-slate-100 focus:ring-2 focus:ring-primary/20 focus:border-primary transition-all font-medium cursor-pointer"
              >
                <option value="topTier" className="dark:bg-slate-900">{t('suppliers.topTier', 'ដៃគូយុទ្ធសាស្ត្រ')}</option>
                <option value="regular" className="dark:bg-slate-900">{t('suppliers.regular', 'ធម្មតា')}</option>
                <option value="new" className="dark:bg-slate-900">{t('suppliers.new', 'ថ្មី')}</option>
              </select>
            </div>
          </div>
          ...
          <div className="grid grid-cols-1 md:grid-cols-3 gap-4">
            ...
            <div>
              ...
              <select
                ...
                className="w-full h-10 min-h-[40px] px-3.5 py-2 text-xs sm:text-[13px] rounded-lg border border-border/80 dark:border-slate-700/80 bg-background dark:bg-slate-900/90 text-foreground dark:text-slate-100 focus:ring-2 focus:ring-primary/20 focus:border-primary transition-all font-medium cursor-pointer"
              >
                <option value="USD" className="dark:bg-slate-900">{t('suppliers.currencyUsd', 'USD ($ - ដុល្លារអាមេរិក)')}</option>
                <option value="KHR" className="dark:bg-slate-900">{t('suppliers.currencyKhr', 'KHR (៛ - រៀលខ្មែរ)')}</option>
                <option value="CNY" className="dark:bg-slate-900">{t('suppliers.currencyCny', 'CNY (¥ - យ័នចិន)')}</option>
                <option value="THB" className="dark:bg-slate-900">{t('suppliers.currencyThb', 'THB (฿ - បាតថៃ)')}</option>
                <option value="VND" className="dark:bg-slate-900">{t('suppliers.currencyVnd', 'VND (₫ - ដុងវៀតណាម)')}</option>
              </select>
            </div>
            ...
            <div>
              ...
              <select
                ...
                className="w-full h-10 min-h-[40px] px-3.5 py-2 text-xs sm:text-[13px] rounded-lg border border-border/80 dark:border-slate-700/80 bg-background dark:bg-slate-900/90 text-foreground dark:text-slate-100 focus:ring-2 focus:ring-primary/20 focus:border-primary transition-all font-medium cursor-pointer"
              >
                <option value="Cash on Delivery" className="dark:bg-slate-900">{t('suppliers.paymentCod', 'ទូទាត់ពេលទទួលទំនិញ')}</option>
                <option value="Net 15" className="dark:bg-slate-900">{t('suppliers.paymentNet15', 'រយៈពេលឥណទាន ១៥ ថ្ងៃ')}</option>
                <option value="Net 30" className="dark:bg-slate-900">{t('suppliers.paymentNet30', 'រយៈពេលឥណទាន ៣០ ថ្ងៃ')}</option>
                <option value="Net 60" className="dark:bg-slate-900">{t('suppliers.paymentNet60', 'រយៈពេលឥណទាន ៦០ ថ្ងៃ')}</option>
                <option value="Advance Payment" className="dark:bg-slate-900">{t('suppliers.paymentAdvance', 'ទូទាត់មុន ១០០%')}</option>
              </select>
            </div>
          </div>
        </div>
        ...
        <div className="bg-card dark:bg-slate-900 border border-border/80 dark:border-slate-800 rounded-2xl p-6 shadow-2xs space-y-5">
          ...
          {contacts.length === 0 ? (
            <div className="text-center py-10 border border-dashed border-border/80 dark:border-slate-800 rounded-xl bg-muted/10 dark:bg-slate-800/40">
              <User size={30} className="mx-auto text-muted-foreground/40 dark:text-slate-500 mb-2" />
              <p className="text-xs sm:text-sm font-semibold text-foreground dark:text-slate-200">
                {t('suppliers.noContactsYet', 'មិនទាន់មានតំណាងទំនាក់ទំនងនៅឡើយទេ។ ចុច "+ បន្ថែមទំនាក់ទំនង" ខាងលើ។')}
              </p>
              <p className="text-[11px] text-muted-foreground dark:text-slate-400 mt-1 max-w-sm mx-auto">
                {t('suppliers.noContactsSub', 'បន្ថែមអ្នកគ្រប់គ្រងផ្នែកលក់ ឬអ្នកសម្របសម្រួលដឹកជញ្ជូន ដើម្បីងាយស្រួលទាក់ទងពេលបញ្ជាទិញ។')}
              </p>
              <button
                type="button"
                onClick={addContactRow}
                className="mt-3 px-3.5 py-1.5 bg-primary text-white text-xs font-bold rounded-lg shadow-xs inline-flex items-center gap-1.5 cursor-pointer hover:opacity-90 transition-opacity"
              >
                <Plus size={14} /> {t('suppliers.addContact', 'បន្ថែមបុគ្គលិកទំនាក់ទំនង')}
              </button>
            </div>
            ...
            <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
              {contacts.map((c, idx) => (
                <div
                  key={idx}
                  className="p-5 bg-muted/20 dark:bg-slate-800/40 border border-border/80 dark:border-slate-700/80 rounded-2xl space-y-4 relative hover:border-border dark:hover:border-slate-600 transition-colors shadow-2xs"
                >
        ...
        <FormFooter
          ...
          infoSummary={
            formData.name ? (
              <span>
                {t('suppliers.tableSupplier', 'អ្នកផ្គត់ផ្គង់')}: <strong className="text-foreground dark:text-slate-100 font-semibold">"{formData.name}"</strong>
              </span>
            ) : (
