      ...
      {isOpen && selectedItem && (
        <div className="fixed inset-0 bg-black/40 z-50 flex justify-end print:static print:bg-transparent">
          ...
          <motion.div
            ...
            className="bg-card dark:bg-slate-900 w-full max-w-xl h-full shadow-2xl relative z-10 p-6 flex flex-col justify-between overflow-y-auto border-l border-border/80 dark:border-slate-800 print:static print:w-full print:p-0 print:shadow-none"
          >
            <div className="space-y-6">
              <div className="flex items-center justify-between border-b border-border/60 dark:border-slate-800 pb-3 print:hidden">
                <h3 className="text-lg font-bold text-foreground dark:text-slate-100">
                  {t('employees.employeeProfileCard', 'Employee Profile Card')}
                </h3>
                <button onClick={onClose} className="text-muted-foreground hover:text-foreground dark:text-slate-400 dark:hover:text-white cursor-pointer">
                  <X size={20} />
                </button>
              </div>

              {/* Profile Card Header */}
              <div className="flex items-center gap-4 bg-muted/40 dark:bg-slate-800/50 p-4 rounded-xl border border-border dark:border-slate-700">
                <EmployeeAvatar
            ...
            <div className="border-t border-border/60 dark:border-slate-800 pt-4 flex gap-2 justify-end print:hidden">
              <button
                onClick={() => {
                  onClose()
                  navigate(`/employees/${selectedItem.id}/edit`)
                }}
                className="h-9 px-4 rounded-xl bg-primary hover:bg-primary/90 text-white text-xs font-bold inline-flex items-center gap-1.5 transition-all shadow-xs cursor-pointer"
              >
                <Edit size={14} />
                <span>{t('employees.editEmployee', 'កែសម្រួល')}</span>
              </button>
              <button
                onClick={onClose}
                className="h-9 px-4 rounded-xl border border-border/80 dark:border-slate-700 bg-card dark:bg-slate-800 text-muted-foreground dark:text-slate-300 hover:text-foreground dark:hover:text-white text-xs font-semibold cursor-pointer"
              >
                {t('common.close', 'បិទ')}
              </button>
            </div>
