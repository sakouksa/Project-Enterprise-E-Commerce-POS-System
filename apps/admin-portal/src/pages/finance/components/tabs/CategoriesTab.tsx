...
export const CategoriesTab: React.FC<CategoriesTabProps> = ({
  ...
  const isAllSelected = categories.length > 0 && allCategoryIds.every((id: number) => selectedRows.includes(id))
            ...
            <tbody className="divide-y divide-border/60">
              {isLoading ? (
                ...
                categories.map((row: any) => {
                  const visual = resolveCategoryVisual(row.name, row.icon, row.color)
                      ...
                      {visibleColumns.category_name && (
                        <td className="px-4 py-3.5">
                          <div className="flex items-center gap-3">
                            <div className={`w-9 h-9 rounded-xl flex items-center justify-center font-bold shrink-0 border ${visual.colorDef.bg} ${visual.colorDef.text} ${visual.colorDef.border}`}>
                              <Icon size={16} />
                            </div>
                            <div className="min-w-0">
                              ...
                              <span className="text-[11px] text-muted-foreground font-medium truncate block">
                                {row.description ? row.description : `#{row.id} • ${t('finance.tab_categories', 'Expense Category')}`}
                              </span>
                            </div>
                          </div>
                        </td>
                        ...
                        <td className="px-4 py-3.5">
                          {row.code ? (
                            <button
                              ...
                              className="inline-flex items-center gap-1.5 px-2.5 py-1 rounded-lg text-xs font-mono font-bold bg-muted/60 hover:bg-muted text-foreground border border-border/80 transition-colors group/btn cursor-pointer"
                            ...
                            >
                              ...
                              {copiedCode === row.code ? (
                                ...
                                <Copy size={11} className="text-muted-foreground group-hover/btn:text-foreground transition-colors" />
                              )}
                            </button>
                          ) : (
