    ...
    <>
      ...
      >
        <DetailDrawerHeader
          ...
          badge={
            <div className="flex items-center gap-1.5">
              ...
              {cust.is_credit_hold && (
                <span className="px-2 py-0.5 rounded-md bg-rose-500/10 text-rose-600 text-[11px] font-bold border border-rose-500/30 flex items-center gap-1">
                  <ShieldAlert size={12} />
        ...
        <DetailDrawerBody isLoading={isLoading}>
          ...
          ════════════════════════════════════════════════════════════════════ */}
          <div className="p-5 rounded-2xl bg-gradient-to-br from-primary/10 via-card to-purple-500/5 border border-primary/20 shadow-2xs space-y-4">
            ...
            <div className="flex items-start gap-4">
              ...
              <div className="flex-1 min-w-0 space-y-1.5">
                ...
                <div className="flex flex-wrap items-center gap-1.5 text-xs text-muted-foreground">
                  ...
                  <span className="inline-flex items-center gap-1 px-2 py-0.5 rounded-md bg-indigo-500/10 text-indigo-600 font-mono font-bold">
            ...
            <div className="flex flex-wrap items-center gap-2.5 pt-3 border-t border-border/60">
              {outstandingNum > 0 && (
                <button
                  ...
                  className="h-9 px-3.5 text-xs font-bold rounded-xl bg-rose-600 text-white hover:bg-rose-700 transition-all flex items-center gap-1.5 shadow-2xs hover:shadow-xs cursor-pointer active:scale-95"
                >
                  <CreditCard size={14} />
                  <span>{t('customers.settleDebt', 'Settle Debt')}</span>
                </button>
              )}
              <button
                type="button"
                onClick={() => setStatementPrintModalOpen(true)}
                className="h-9 px-3.5 rounded-xl border border-border/80 bg-background text-foreground hover:bg-muted text-xs font-bold transition-all shadow-2xs cursor-pointer flex items-center gap-2 active:scale-95"
              >
                <FileText size={15} className="text-primary" />
                <span>{t('customers.printSOA', 'Statement (SOA)')}</span>
              </button>
              <button
                type="button"
                onClick={() => setTopupModalOpen(true)}
                className="h-9 px-3.5 text-xs font-bold rounded-xl bg-emerald-600 text-white hover:bg-emerald-700 transition-all flex items-center gap-1.5 shadow-2xs hover:shadow-xs cursor-pointer active:scale-95"
              >
                <Wallet size={14} />
              ...
              <button
                ...
                className="h-9 px-3.5 text-xs font-bold rounded-xl bg-primary text-primary-foreground hover:bg-primary/90 transition-all flex items-center gap-1.5 shadow-2xs hover:shadow-xs cursor-pointer active:scale-95"
              >
                <PhoneCall size={14} />
                <span>{t('customers.logCall', 'Log Activity')}</span>
              </button>
            </div>
          ...
          {/* ════════════════════════════════════════════════════════════════════
              TAB NAVIGATION (Scrollable 10 Enterprise Tabs)
          ════════════════════════════════════════════════════════════════════ */}
          <div className="pt-2">
            <DetailDrawerTabNav
              tabs={tabs}
          ...
          <div className="mt-4">
              ...
              <div className="space-y-4">
                ...
                <div className="p-4 rounded-xl bg-card border border-border space-y-3">
                  <h3 className="text-xs font-bold uppercase tracking-wider text-muted-foreground flex items-center gap-1.5">
                    ...
                    {t('customers.b2bEntityProfile', 'B2B Entity & Communication Profile')}
                  </h3>

                  <div className="grid grid-cols-1 sm:grid-cols-2 gap-4 text-xs">
                    <div className="space-y-1">
                      <span className="text-muted-foreground">{t('customers.directEmail', 'Direct Email:')}</span>
                      <div className="flex items-center justify-between font-mono p-2 rounded-lg bg-muted/50 border border-border/60">
                        <span className="truncate">{cust.email || 'N/A'}</span>
                        {cust.email && (
                          <button onClick={() => copyToClipboard(cust.email!, 'email')} className="p-1 hover:text-primary cursor-pointer">
                    ...
                    <div className="space-y-1">
                      <span className="text-muted-foreground">{t('customers.primaryPhone', 'Primary Phone:')}</span>
                      <div className="flex items-center justify-between font-mono p-2 rounded-lg bg-muted/50 border border-border/60">
                        <span>{cust.phone || 'N/A'}</span>
                        {cust.phone && (
                          <button onClick={() => copyToClipboard(cust.phone!, 'phone')} className="p-1 hover:text-primary cursor-pointer">
                    ...
                    <div className="space-y-1">
                      <span className="text-muted-foreground">{t('customers.vatTaxId', 'VAT / Tax ID:')}</span>
                      <div className="p-2 rounded-lg bg-muted/50 border border-border/60 font-mono">
                        {cust.tax_number || t('customers.noTaxId', 'No Tax ID Registered')}
                      </div>
                    </div>

                    <div className="space-y-1">
                      <span className="text-muted-foreground">{t('customers.taxBranchCode', 'Tax Branch Code:')}</span>
                      <div className="p-2 rounded-lg bg-muted/50 border border-border/60 font-mono">
                        {cust.tax_branch_code || t('customers.headOffice', 'Head Office')}
                      </div>
                    </div>
                  </div>
                </div>
                ...
                {cust.notes && (
                  <div className="p-4 rounded-xl bg-amber-500/5 border border-amber-500/20 text-xs space-y-1.5">
                    <p className="font-bold text-amber-700 dark:text-amber-400 flex items-center gap-1.5">
                      <FileText size={14} />
                      {t('customers.accountNotes', 'Special Customer Directives & Account Notes')}
                    </p>
                    <p className="text-muted-foreground leading-relaxed">{cust.notes}</p>
                  </div>
                )}
              </div>
            )}
            ...
            {activeTab === 'credit_wallet' && (
              <div className="space-y-4">
                ...
                <div className="p-5 rounded-2xl bg-card border border-border/80 shadow-xs space-y-5">
                  {/* Card Header */}
                  <div className="flex items-center justify-between flex-wrap gap-3 pb-3.5 border-b border-border/60">
                    <div className="flex items-center gap-3">
                      ...
                      <div>
                        <h3 className="text-sm font-bold text-foreground">
                          {t('customers.creditFacilityTitle', 'B2B Credit Facility & Aging')}
                        </h3>
                        <div className="flex items-center gap-2 mt-0.5">
                          <span className="text-xs text-muted-foreground">{t('customers.termsLabel', 'Terms:')}</span>
                          <span className="px-2 py-0.5 rounded-md bg-muted text-foreground text-[11px] font-bold uppercase font-mono border border-border/60">
                            {cust.payment_terms || 'PREPAID'}
                          </span>
                        </div>
                      </div>
                    </div>
                    ...
                    <button
                      ...
                      className={`h-9 px-3.5 text-xs font-bold rounded-xl border transition-all flex items-center gap-1.5 cursor-pointer shadow-2xs active:scale-95 ${
                        cust.is_credit_hold
                          ? 'bg-rose-500 text-white border-rose-600 hover:bg-rose-600'
                          : 'bg-card hover:bg-rose-500/10 text-muted-foreground hover:text-rose-600 border-border/80'
                      }`}
                    >
                      <ShieldAlert size={14} className={cust.is_credit_hold ? 'text-white' : 'text-rose-500'} />
                      <span>{cust.is_credit_hold ? t('customers.creditLockedUnlock', 'Credit Locked (Click to Unlock)') : t('customers.lockCreditHold', 'Lock Account / Place on Hold')}</span>
                    </button>
                  </div>
                ...
                <div className="p-5 rounded-2xl bg-card border border-border/80 shadow-xs space-y-3.5">
                  <div className="flex items-center justify-between flex-wrap gap-2 pb-2.5 border-b border-border/60">
                    <div className="flex items-center gap-2.5">
                      ...
                      <div>
                        <span className="text-xs font-bold text-muted-foreground">{t('customers.storeWalletBalance', 'Store Wallet Balance')}</span>
                    ...
                    <button
                      type="button"
                      onClick={() => setTopupModalOpen(true)}
                      className="h-8 px-3 text-xs font-semibold rounded-lg bg-emerald-50 dark:bg-emerald-950/40 text-emerald-700 dark:text-emerald-300 border border-emerald-500/30 hover:bg-emerald-100 dark:hover:bg-emerald-900/50 transition-colors flex items-center gap-1.5 cursor-pointer shadow-2xs active:scale-95 shrink-0"
                    >
                      <Plus size={13} />
                      <span>{t('customers.topUpAdjust', '+ Top Up / Adjust')}</span>
                    </button>
                  </div>
                        ...
                        {walletList.length === 0 ? (
                           <tr>
                             <td colSpan={5} className="text-center py-8 text-muted-foreground bg-card">
                               <Wallet size={24} className="mx-auto mb-2 opacity-30 text-muted-foreground" />
            ...
            {activeTab === 'loyalty' && (
              <div className="space-y-4">
                {/* Executive Tier Card */}
                <div className="p-5 rounded-2xl bg-gradient-to-br from-amber-500/10 via-card to-orange-500/5 border border-amber-500/25 shadow-xs space-y-4">
                  <div className="flex items-center justify-between flex-wrap gap-3 pb-3 border-b border-amber-500/20">
                    <div className="flex items-center gap-3">
                      <div className="w-11 h-11 rounded-2xl bg-amber-500/20 text-amber-700 dark:text-amber-300 font-extrabold text-sm flex items-center justify-center border border-amber-500/30 shadow-xs shrink-0">
                        <Award size={22} className="text-amber-600 dark:text-amber-400" />
                      </div>
                      <div>
                        <div className="flex items-center gap-2">
                          <span className="text-xs text-muted-foreground font-medium">{t('customers.membershipTier', 'Membership Tier:')}</span>
                          <span className="px-2.5 py-0.5 rounded-lg bg-amber-500/20 text-amber-800 dark:text-amber-300 font-extrabold text-xs tracking-wider uppercase border border-amber-500/30">
                            {currentTier} MEMBER
                          </span>
                        </div>
                    ...
                    <button
                      ...
                      className="h-9 px-3.5 text-xs font-bold rounded-xl bg-amber-500 text-white hover:bg-amber-600 transition-colors flex items-center gap-1.5 cursor-pointer shadow-2xs active:scale-95"
                    >
                      <Plus size={13} />
                ...
                <div className="p-5 rounded-2xl bg-card border border-border/80 shadow-xs space-y-3.5">
                  <div className="flex items-center justify-between flex-wrap gap-2 pb-2.5 border-b border-border/60">
                    <div className="flex items-center gap-2.5">
                      ...
                      <div>
                        <span className="text-xs font-bold text-muted-foreground">{t('customers.pointsBalance', 'Total Points Balance')}</span>
                    ...
                    <table className="w-full text-xs">
                      ...
                      <tbody className="divide-y divide-border/40">
                        {pointsList.length === 0 ? (
                          <tr>
                            <td colSpan={5} className="text-center py-8 text-muted-foreground bg-card">
                              <Award size={24} className="mx-auto mb-2 opacity-30 text-muted-foreground" />
              ...
              <div className="space-y-4">
                <div className="flex items-center justify-between">
                  <p className="text-xs text-muted-foreground">
                    {t('customers.corporateContactsTitle', 'Corporate Contact Persons & Department Hierarchy')}
                  </p>
                  <button
                    type="button"
                    onClick={() => setContactModalOpen(true)}
                    className="px-3 py-1.5 text-xs font-bold rounded-xl bg-primary text-primary-foreground hover:bg-primary/90 flex items-center gap-1.5 shadow-2xs cursor-pointer"
                  >
                    <Plus size={13} />
                ...
                <div className="grid grid-cols-1 sm:grid-cols-2 gap-3">
                  {contactsList.length === 0 ? (
                    <div className="col-span-2 text-center py-8 text-muted-foreground text-xs bg-card rounded-2xl border border-border">
                    ...
                    contactsList.map((c) => (
                      <div key={c.id} className="p-4 rounded-xl bg-card border border-border space-y-2.5 shadow-2xs relative">
                        <div className="flex items-start justify-between">
                          ...
                          <button
                            ...
                            className="text-muted-foreground hover:text-rose-500 text-[11px] cursor-pointer"
                          >
                            <X size={14} />
                          </button>
                        </div>
            ...
            {activeTab === 'pricing' && (
              <div className="space-y-4">
                <div className="flex items-center justify-between">
                  <p className="text-xs text-muted-foreground">
                    {t('customers.pricingContractsTitle', 'Custom Negotiated Price Lists & Volume Agreements')}
                  </p>
                </div>

                <div className="space-y-3">
                  {contractsList.length === 0 ? (
                    <div className="text-center py-8 text-muted-foreground text-xs bg-card rounded-2xl border border-border">
            ...
            {activeTab === 'timeline' && (
              <div className="space-y-4">
                <div className="flex items-center justify-between">
                  <p className="text-xs text-muted-foreground">
                    {t('customers.timelineTitle', 'Omnichannel Communication Log & Interaction History')}
                  </p>
                  <button
                    type="button"
                    onClick={() => setInteractionModalOpen(true)}
            ...
            {activeTab === 'tickets' && (
              <div className="space-y-4">
                <div className="flex items-center justify-between">
                  <p className="text-xs text-muted-foreground">
                    {t('customers.supportTicketsTitle', 'Customer Care, RMA Returns & Warranty Tickets')}
                  </p>
                  <button
                    type="button"
            ...
            {activeTab === 'kyc' && (
              <div className="space-y-4">
                <div className="flex items-center justify-between">
                  <p className="text-xs text-muted-foreground">
                    {t('customers.kycComplianceTitle', 'Corporate Compliance, Tax Certificates & Signed Contracts')}
                  </p>
                  <button
                    type="button"
            ...
            {activeTab === 'orders' && (
              <div className="space-y-4">
                <div className="overflow-x-auto">
                  <table className="w-full text-xs">
                    <thead className="bg-muted/50 text-[10px] uppercase text-muted-foreground font-bold border-b border-border">
                      <tr>
                        <th className="py-2.5 px-2 text-left">{t('customers.invoiceNo', 'Invoice No')}</th>
                        <th className="py-2.5 px-2 text-left">{t('common.date', 'Date')}</th>
                        <th className="py-2.5 px-2 text-right">{t('customers.items', 'Items')}</th>
                        <th className="py-2.5 px-2 text-right">{t('customers.grandTotal', 'Grand Total')}</th>
                        <th className="py-2.5 px-2 text-center">{t('common.status', 'Status')}</th>
                      </tr>
                    </thead>
                    <tbody>
                      {ordersList.length === 0 ? (
                        ...
                        ordersList.map((sale: any) => (
                          <tr key={sale.id} className="border-b border-border/40 hover:bg-muted/20">
                ...
                <div className="grid grid-cols-1 sm:grid-cols-2 gap-3">
                  {addressesList.length === 0 ? (
                    ...
                    addressesList.map((addr) => (
                      <div key={addr.id} className="p-4 rounded-xl bg-card border border-border space-y-2 shadow-2xs">
                        <div className="flex items-center justify-between">
        ...
        <DetailDrawerFooter>
          <div className="flex items-center justify-between w-full">
            <button
              type="button"
              onClick={onClose}
              className="h-10 px-5 text-xs sm:text-[13px] font-bold rounded-xl border border-border/80 bg-card hover:bg-muted text-foreground transition-all shadow-2xs cursor-pointer active:scale-95"
            >
            ...
            <button
              ...
              className="h-10 px-5 text-xs sm:text-[13px] font-bold rounded-xl bg-primary text-primary-foreground hover:bg-primary/90 transition-all flex items-center gap-2 shadow-xs hover:shadow cursor-pointer active:scale-95"
            >
              <Edit3 size={15} />
